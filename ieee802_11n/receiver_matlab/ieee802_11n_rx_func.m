...
for i=1:size(rx_signal,2)
    ...
    axis([1,size(rx_signal,1),-32768,32767]);
end
title('原始信号时域波形');
hold off
subplot(242);
...
sim_options.sync_index=thres_idx+thres_idx_long;
sim_options.freq_est=freq_est;
%% spilt stf,ltf,data
...
if crc_outputs==0
    ...
    cyc=cyc+1;
    evm(cyc,:)=evm_data;
    sim_options.evm_data=evm_data;
    disp(num2str(20*log10(evm_data),3));
else
    crc_ok='NO';
    err_cyc=err_cyc+1;
end
